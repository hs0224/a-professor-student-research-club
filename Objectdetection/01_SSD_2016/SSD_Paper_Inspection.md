...
* [19] 참고 문헌을 이용한 VOC2007 테스트에서 사용된 SSD 512 모델의 성능 도표들로서
    ...
    + (2) 성능 도표들 중 첫번 째 행을 분석해 보았을 때,
        - (2-1) 도표의 x축은 '총 탐지된 object들의 수(?)'를 나타내는 것으로 추정할 수 있다. '(x 357)', '(x 415)' 등으로 적힌 것으로 보아 357개, 415개의 객체들이 탐지된 것으로 보인다.
    ...
    + (?) 총 탐지된 object들의 수-total detections, 각 유형별 퍼센트-percentage of each type
...
> * 세 열을 좌측부터 순서대로 보았을 때, 총 탐지 수를 고려하고 보아도, furniture의 FP 면적이 제일 크게 나타나는 것을 볼 수 있다. 또한, 세 도표들 중 빨간 실선이 가장 완만하게 상승한다.
...
* 다중 계층의 영향에 대한 표로 Fig 2. 모델 구조와 비교해 보는 것을 권장한다.
    ...
    + (8) 마지막 열 # Boxes 열은 만들어진 Box들의 개수를 의미한다.
        - (8-1) 탐지된 객체들의 박스 수를 의미하는 것으로 추정된다.
...
* Pascal VOC2007 테스트 결과에 대한 표로
    ...
    + (4) # Boxes 열이 존재한다. 이는 탐지된 객체 박스들의 개수를 의미한다.
...
* (1) 본문은 '단일' 신층 신경망을 사용해 이미지 내 객체들을 탐지하는 방법(모델)을 제공(제시)한다.
...
* (5) (4)의 특성은 SSD가 학습하는데 쉽게 해주고 '탐지 구성(?)'이 필요한 시스템에 직관적으로 융합하게 한다.
...
* (1) 최신 객체 detection 시스템은 다음 방법들의 변형이다.
    + (1-1) 방법: BBox 가정하기, 각각의 BBox에서 특성 또는 픽셀을 리샘플하기, 고품질의 분류기 적용하기.
* (2) 이 파이프라인은 Selective Search work [1] 이후로, 예를 들어, 참고문헌 [3]이 더 깊은 특성을 가짐에도 불구하고, 현재 PASCAL VOC, COCO 및 ILSVRC 탐지 모두 Faster R-CNN 기반의 선행 결과가 탐지 벤치마크들에 만연해 있다.
* (3) Faster R-CNN 기반의 방법들은 정확하기는 하지만 너무 임베디드 시스템에 집중적이고, 심지어 하이엔드 하드웨어를 사용하더라도 실시간 응용프로그램에 사용하기에는 너무 느리다. 종종 FPS로 (심지어 Faster R-CNN 이라는 가장 빠른 고 정확도의 탐지기로) 측정된 이 방법들의 탐지 속도는 단 7 FPS를 나타냈다. 이러한 문제를 해결하기 위해, 탐지 파이프라인의 각 단계에 달려들어 더 빠른 탐지기를 만들기 위해 많은 시도를 해왔다. 하지만, 여태까지 현저하게 증가된 속도는 그 대가로 현저하게 떨어진 탐지 정확도를 도출할 뿐이었다.

* (4) 본 논문은 우선 픽셀 리샘플 또는 BBox 가정을 하지 않는 객체 탐지 기반 심층 신경망을 제공한다.
* (5) 본 논문에서 제시하는 신경망은 결과적으로 고정밀 탐지의 속도 면에서 두드러지는 개선 사항이 되었다.
    + (5-1) VOC2007 test에서 Faster R-CNN은 7 FPS와 73.2% mAP를, YOLO는 45 FPS와 63.4% mAP를 기록한 반면, 본 신경망은 59 FPS와 74.3% mAP 를 탐지했다.
* (6) 속도 개선의 근본적인 내용은 BBox 제안, 후속 픽셀 단계, 특성 리샘플 단계 등을 제거함으로써 도출되었다. 이러한 시도를 한 연구는 본 연구가 최초는 아니지만(참고 문헌 [4, 5] 참조), 일련의 개선 사항을 추가함으로써 이전 시도들에 비해 두드러지는 정확도 향상을 이루어냈다.
* (7) 일련의 개선안들은 아래 내용들이 포함되어 있다.
    + (7-1) BBox 위치 내에서 객체의 범주들과 위치좌표를 예측하기 위한 작은 합성곱 필터들을 사용한다.
    + (7-2) 서로 다른 종횡비 탐지를 위한 개별적인 예측기들(작은 합성곱 필터들)을 사용한다.
    + (7-3) 네트워크(신경망)의 나중 단계에 있는 '다양한 크기들에서 탐지를 수행하기 위한 여러개의 특성 맵들'에 이러한 필터들을 적용한다.
* (8) 특히 다양한 크기에서의 예측을 위한 다중 계층을 사용하는 이러한 변경점들(개선안들)로 연구진은 상대적으로 낮은 해상도 입력을 사용하더라도 탐지 속도 향상과 높은 정확도를 달성했다.
* (9) 이러한 기여(공헌)들이 별로 독립적이지 않아 보이지만(?), 본 연구는 PASCAL VOC에서 SSD가 63.4% mAP를 보인 YOLO와 비교해 74.3% mAP로 실시간 탐지 정확도를 개선해낸 시스템임을 주목해야한다. 이는 최근 탐지 정확도에 비해 매우 큰 개선으로 나머지 신경망들 [3]에 비해 이목을 끄는 성과 이다. 게다가, 고품질 탐지에서 두드러지는 속도 개선은 컴퓨터 비전에 유용한 구성 범위들을 넓힐 수 있었다.

다음은 연구진들이 이뤄낸 성과를 요약한 글에 대한 해석이다.

* (10) SSD를 소개한다. 사실상, (Faster R-CNN을 포함해) 분명한 지역 제안과 풀링을 수행하는 더 느린 기술들의 정확도 만큼 확실히 더 정확하고, 이전 최신 싱글 샷 디텍터인 YOLO 보다 빠르다.
* (11) SSD의 핵심은 고정된 기본 BBox 뭉치들을 위해서 범주 점수들과 박스 위치 좌표를 예측하는 것이다.
    +(11-1) 기본 BBox 뭉치들은 특성 맵들에 적용된 작은 합성곱 필터를 사용한다.
* (12) 높은 탐지 정확도를 달성하기 위해, 다양한 크기의 특성맵으로부터 다양한 크기의 예측을 만들어 낸다(수행한다). 그리고, 종횡비를 수단으로 삼아 예측들을 분명히 분류한다.
* (13) 이러한 디자인 특성들은 간단한 끝과 끝이 맞닿는(연속적인(?)) 학습과 높은 정확도를 이끌어 냈다.
    + (13-1) 낮은 해상도의 입력 이미지에서도 높은 정확도를 보였다.
    + (13-2) 게다가 속도-정확도 trade-off의 개선을 이끌어 냈다.
* (14) 실험들은 PASCAL VOC, COCO, ILSVRC 등에서 평가된 다양한 입력 크기의 모델들의 정확도 분석과 타이밍을 포함한다.
